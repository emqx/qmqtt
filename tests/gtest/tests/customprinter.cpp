...
void CustomPrinter::OnTestIterationStart(const UnitTest& unit_test, int iteration)
{
    ...
    qout << "[==========] Running "
         << (unit_test.test_to_run_count() == 1 ? "test" : "tests")
         << " from "
...
void CustomPrinter::OnTestIterationEnd(const UnitTest& unit_test, int /*iteration*/)
{
    qout << "[==========] "
         << (unit_test.test_to_run_count() == 1 ? "test" : "tests")
         << " from "
         << (unit_test.test_case_to_run_count() == 1 ? "test case" : "test cases")
         << (GTEST_FLAG(print_time) ? QString("(%1 ms total)").arg(unit_test.elapsed_time()): "")
         << endl;

    qout << "[  PASSED  ] "
         << (unit_test.successful_test_count() == 1 ? "test." : "tests.")
         << endl;
